...
export function ContactForm() {
  const [f, setF] = useState({ name: '', phone: '', service: 'Clay bar + full detail', message: '' })
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-4">
      ...
      <select value={f.service} onChange={(e) => setF({ ...f, service: e.target.value })} className={field}>
        {['Clay bar + full detail', 'Exterior wash & wax', 'Interior deep clean', 'Ceramic coating', 'Paint correction', 'Wheels & paint detail', 'Not sure — recommend something'].map((o) => <option key={o}>{o}</option>)}
      </select>
