    ...
    <header className={`fixed inset-x-0 top-0 z-50 transition-all duration-500 ease-[var(--ease-quint)] ${scrolled ? 'bg-bg/85 backdrop-blur-md border-b border-line' : 'bg-transparent'}`}>
      <div className="mx-auto flex h-[72px] max-w-shell items-center justify-between px-5 lg:px-10">
        ...
        <nav className="hidden items-center gap-9 md:flex">
          ...
          <a href="tel:+14804185533" className="inline-flex items-center gap-2 rounded-full bg-accent px-5 py-2.5 text-sm font-medium text-bg transition-colors duration-500 ease-[var(--ease-quint)] hover:bg-accent-dark">
            <Phone size={15} /> (480) 418-5533
          </a>
        </nav>
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex flex-col bg-bg md:hidden">
          ...
          <nav className="flex flex-1 flex-col justify-center gap-2 px-6">
            ...
            <a href="tel:+14804185533" className="mt-8 inline-flex w-fit items-center gap-2 rounded-full bg-accent px-6 py-3 font-medium text-bg">
              <Phone size={17} /> (480) 418-5533
            </a>
          </nav>
        </div>
      )}
