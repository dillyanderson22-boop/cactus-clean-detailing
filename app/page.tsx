...
import { Phone, ArrowUpRight, Star, MapPin, Sparkles, Car, Droplets, ShieldCheck, CheckCircle2 } from 'lucide-react'
  ...
  return (
    <>
      ...
      <section className="relative flex h-[100svh] min-h-[640px] w-full items-end overflow-hidden bg-bg2 grain">
        <Image src={px(photos.hero, 2000)} alt="Detailer claying and polishing a car in the East Valley" fill priority sizes="100vw" className="object-cover object-center" />
        ...
        <div className="relative z-10 mx-auto w-full max-w-shell px-5 pb-16 lg:px-10 lg:pb-24">
          ...
          <h1 className="mt-6 max-w-4xl font-display text-[clamp(46px,8vw,118px)] font-semibold uppercase leading-[0.92] tracking-tight">
            Glass-smooth paint, <span className="text-accent">in your driveway.</span>
          </h1>
          <p className="mt-6 max-w-xl text-lg leading-relaxed text-fg/85">
            A proper clay-bar treatment, ceramic coating and full detail — done at your home or office. We pull the embedded grit Arizona roads leave behind and leave your paint slick and protected.
          </p>
      ...
      <div className="border-y border-line bg-bg2">
        <div className="mx-auto flex max-w-shell flex-wrap items-center justify-between gap-x-10 gap-y-4 px-5 py-5 text-sm lg:px-10">
          ...
          <span className="inline-flex items-center gap-2 text-fg/80"><Sparkles size={16} className="text-accent" /> Clay bar & ceramic specialists</span>
        ...
        <div className="mb-16 flex flex-col justify-between gap-6 md:flex-row md:items-end">
          <div>
            ...
            <h2 data-reveal className="mt-4 max-w-2xl font-display text-5xl font-semibold uppercase leading-[0.95] tracking-tight md:text-7xl">
              Detailing that goes deeper than a wash.
            </h2>
          </div>
          <p data-reveal className="max-w-sm text-muted">
            Every job is done by hand with pro-grade products — at your place, on your schedule.
          </p>
        </div>
        ...
        <div className="grid gap-5 md:grid-cols-12">
          <div data-reveal className="group relative overflow-hidden rounded-2xl md:col-span-7">
            <div className="relative aspect-[16/11] w-full">
              <Image src={px(photos.clay, 1400)} alt="Clay bar treatment removing embedded contaminants from paint" fill sizes="(max-width:768px) 100vw, 58vw" className="object-cover transition-transform duration-[1.2s] ease-[var(--ease-quint)] group-hover:scale-[1.04]" />
              <div className="absolute inset-0 bg-gradient-to-t from-bg via-bg/10 to-transparent" />
            </div>
            <div className="absolute bottom-0 p-7">
              <Sparkles className="mb-3 text-accent" />
              <h3 className="font-display text-3xl font-semibold uppercase tracking-tight md:text-4xl">Clay bar &amp; decontamination</h3>
              <p className="mt-2 max-w-md text-sm text-fg/80">Our signature step. We lift the embedded grit, overspray and hard-water deposits a normal wash leaves behind, so paint feels like glass and coatings actually bond.</p>
            </div>
          </div>

          <div className="flex flex-col gap-5 md:col-span-5">
            {[
              { icon: ShieldCheck, t: 'Ceramic coating', d: 'Long-lasting gloss and protection from Arizona sun, locked in after a proper clay-bar prep.' },
              { icon: Droplets, t: 'Exterior wash & wax', d: 'Hand wash, clay, wax and tire shine — every panel, wheel and window done right.' },
              { icon: Car, t: 'Interior deep clean', d: 'Vacuum, steam, leather and vinyl conditioning, glass and a full surface wipe-down.' },
            ].map((s) => (
      ...
      <section className="border-y border-line bg-bg2">
        <Reveal className="mx-auto max-w-shell px-5 py-24 lg:px-10">
          <div data-reveal className="kicker mb-12 text-accent">Why book Cactus Clean</div>
      ...
      <Reveal className="mx-auto max-w-shell px-5 py-28 lg:px-10 lg:py-36">
        ...
        <div className="mt-16 grid gap-px overflow-hidden rounded-2xl border border-line bg-line md:grid-cols-3">
          {[
            ...
            { n: '03', t: 'Feel the difference', d: 'Run your hand across the paint when we’re done. Clay-bar smooth, sealed and ready for the sun.' },
          ].map((s) => (
        ...
        <div className="grid grid-cols-2 gap-3 md:grid-cols-3 md:gap-4">
          {photos.gallery.slice(0, 6).map((id, i) => (
            <div key={id} data-reveal className={`group relative overflow-hidden rounded-xl ${i === 0 ? 'col-span-2 row-span-2 aspect-square md:aspect-auto' : 'aspect-[4/3]'}`}>
              <Image src={px(id, 900)} alt="Cactus Clean Detailing result" fill sizes="(max-width:768px) 50vw, 33vw" className="object-cover transition-transform duration-[1.2s] ease-[var(--ease-quint)] group-hover:scale-[1.05]" />
            </div>
          ))}
        </div>
      </Reveal>
      ...
      <section className="border-y border-line bg-bg2">
        <Reveal className="mx-auto max-w-shell px-5 py-24 lg:px-10">
          <div data-reveal className="kicker mb-12 text-accent">The Cactus Clean promise</div>
          <div className="grid gap-6 md:grid-cols-3">
            {[
              { t: 'We come to you', d: 'Home, office or apartment — we bring our own water and power and set up wherever you park.' },
              { t: 'Clay-bar smooth', d: 'We don’t skip the prep. Decontamination and clay come standard so the finish actually lasts.' },
      ...
      <Reveal className="mx-auto max-w-shell px-5 py-28 text-center lg:px-10 lg:py-36">
        ...
        <p data-reveal className="mx-auto mt-6 max-w-md text-muted">
          Call or text for a quick quote. We&apos;ll bring the clay bar to you anywhere in the East Valley.
        </p>
